...
def test_mnist_checkpoint_resume():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            state2.step == 6
        ), f"Step should be restored: expected 6, got {state2.step}"
...
def test_mnist_microbatch_accumulation():
    ...
    def create_microbatch_data(num_batches=3):
        ...
        for _ in range(num_batches):
            ...
            for i in range(accumulate_steps):
                ...
                microbatches.append(
                    {"x": x_full[start_idx:end_idx], "y": y_full[start_idx:end_idx]}
                )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert (
            final_loss < initial_loss
        ), f"Loss should decrease with microbatches: {initial_loss} -> {final_loss}"
        print(
            f"Microbatch accumulation test passed: {initial_loss:.4f} -> {final_loss:.4f}"
        )
