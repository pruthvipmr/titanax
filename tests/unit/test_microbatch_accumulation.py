...
class TestGradientAccumulation:
    ...
    def simple_loss_fn(self, params, batch):
        ...
        x, y = batch["x"], batch["y"]
        pred = jnp.dot(x, params["weight"]) + params["bias"][0]  # Use first bias element
    ...
    def test_multiple_steps_accumulation(self):
        ...
        assert not jnp.allclose(result_state.params["weight"], self.state.params["weight"])
    ...
    def test_gradient_accumulation_mathematical_correctness(self):
        ...
        assert jnp.allclose(accumulated_state.params["weight"], manual_state.params["weight"], atol=1e-6)
        assert jnp.allclose(accumulated_state.params["bias"], manual_state.params["bias"], atol=1e-6)
...
class TestStepFnCreation:
    ...
    def test_create_step_fn_with_accumulation(self):
        ...
        with pytest.raises(EngineError, match="Gradient accumulation requires batch to contain 'microbatches'"):
