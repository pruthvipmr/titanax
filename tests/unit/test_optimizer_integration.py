...
class TestTrainStateOptimizerIntegration:
    ...
    def test_apply_gradients_with_stored_optimizer(self):
        ...
        assert not jnp.allclose(
            new_state.params["layer1"]["weight"], self.state.params["layer1"]["weight"]
        )
    ...
    def test_apply_gradients_with_explicit_optimizer(self):
        ...
        new_state = state_with_explicit_opt.apply_gradients(
            grads=grads, optimizer=explicit_optimizer
        )
...
class TestEngineOptimizerIntegration:
    ...
    def setup_method(self):
        ...
        self.params = {"weight": jnp.array([[1.0, 2.0]]), "bias": jnp.array([0.1])}
    ...
    def test_learning_rate_metrics(self):
        ...
        assert metrics["learning_rate"] == self.optimizer.get_learning_rate(
            new_state.step
        )
