    ...
    def _validate(self) -> None:
        ...
        if not self.axis:
            raise plan_validation_error(
                "DP axis cannot be empty",
                "Provide a valid axis name like 'data'"
            )
...
class TP:
    ...
    def _validate(self) -> None:
        ...
        if not self.axis:
            raise plan_validation_error(
                "TP axis cannot be empty",
                "Provide a valid axis name like 'model'"
            )
    ...
    def describe(self) -> str:
        ...
        description = f"Tensor Parallel on axis '{self.axis}' with {len(self.rules)} rules"
...
class PP:
    ...
    def _validate(self) -> None:
        ...
        if not self.axis:
            raise plan_validation_error(
                "PP axis cannot be empty",
                "Provide a valid axis name like 'pipe'"
            )
    ...
    def describe(self) -> str:
        ...
        description = f"Pipeline Parallel on axis '{self.axis}' with {len(self.stages)} stages"
...
class Plan:
    ...
    def _validate_composition(self) -> None:
        ...
        if self.pipeline_parallel:
            if self.pipeline_parallel.axis in used_axes:
                ...
                if (self.tensor_parallel and 
                    self.pipeline_parallel.axis == self.tensor_parallel.axis):
    ...
    def is_data_parallel_only(self) -> bool:
        ...
        return (self.data_parallel is not None and 
                self.tensor_parallel is None and 
                self.pipeline_parallel is None)
    
    def has_microbatching(self) -> bool:
        """Check if this plan uses microbatching (DP accumulation or PP)."""
        dp_microbatching = (self.data_parallel and 
                           self.data_parallel.accumulate_steps > 1)
