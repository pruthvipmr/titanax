...
def resolve_checkpoint_step(
    strategy: CheckpointStrategy, step: Optional[int] = None
) -> int:
...
def validate_checkpoint_compatibility(
    ...
    if strict:
        if (
            current_mesh_spec is not None
            and checkpoint_metadata.mesh_spec != current_mesh_spec
        ):
            raise CheckpointError(
        ...
        if (
            current_plan_spec is not None
            and checkpoint_metadata.plan_spec != current_plan_spec
        ):
            raise CheckpointError(
