...
def initialize_distributed(
    ...
    if not all(
        [coordinator_address, process_count is not None, process_id is not None]
    ):
    ...
    try:
        logger.info(
            f"Initializing JAX distributed: coordinator={coordinator_address_with_port}, "
        ...
        logger.info(
            f"JAX distributed initialized successfully. "
...
def enumerate_devices(
    device_type: Optional[str] = None, local_only: bool = False
) -> List[Device]:
