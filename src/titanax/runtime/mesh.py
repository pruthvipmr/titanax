...
class MeshSpec:
    ...
    def _validate_spec(self) -> None:
        ...
        if len(set(self.axes)) != len(self.axes):
            raise mesh_validation_error(
                f"Duplicate axis names in {self.axes}",
                "Each axis name must be unique"
            )
        
        for axis in self.axes:
            if not isinstance(axis, str) or not axis:
                raise mesh_validation_error(
                    f"Invalid axis name: {axis}",
    ...
    def _resolve_devices(self) -> List[Device]:
        ...
        if not devices:
            raise mesh_validation_error(
                ...
                "Ensure JAX can access at least one device"
            )
    ...
    def validate_compatibility(self, batch_size: int, allow_padding: bool = False) -> None:
        ...
        try:
            ...
            if batch_size % data_parallel_size != 0:
                if not allow_padding:
                    ...
                    padded_batch = ((batch_size + data_parallel_size - 1) // data_parallel_size) * data_parallel_size
                    padding = padded_batch - batch_size
                    if padding > 0:
                        # This is just a warning, not an error
                        import logging
                        logging.warning(f"Batch size {batch_size} will be padded to {padded_batch} "
